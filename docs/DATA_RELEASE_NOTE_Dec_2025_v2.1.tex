...
\textbf{Total Measurements (after QC)}: 13,231,288 (2,443 samples $\times$ 5,416 proteins) \\
...
Contains all 84 samples flagged as outliers by any QC method, with detailed QC flags and metrics for each method.
...
Primary analysis dataset containing 2,443 samples (rows) $\times$ 5,416 proteins (columns) that passed all QC filters. Contains raw NPX values. \textbf{Note}: 24 control probes (8 incubation, 8 extension, 8 amplification) are excluded from released data, leaving 5,416 biological proteins.
...
\textbf{Total technical outliers flagged}: 33 samples (1.31\% of 2,527 analysis-ready samples)
...
1. \textbf{Strict Mismatches} (predicted\_sex $\neq$ genetic\_sex):
   \begin{itemize}
     ...
     \item \textbf{Strict mismatches flagged}: 23 samples (0.92\% of 2,505 PCA-cleaned samples)
   \end{itemize}

2. \textbf{Sex Outliers} (threshold-based deviations, NOT strict mismatches):
   \begin{itemize}
     \item \textbf{Definition}: Samples where predicted female probability deviates from expected thresholds (Youden's J = 0.71 or 0.5) but predicted\_sex still matches genetic\_sex
     \item \textbf{Logic}:
       \begin{itemize}
         \item Males: \texttt{predicted\_prob $\geq$ 0.71} (Youden's J) OR \texttt{0.5 $\leq$ predicted\_prob $<$ 0.71} (between 0.5 and Youden's J)
         \item Females: \texttt{predicted\_prob $<$ 0.71} (Youden's J)
       \end{itemize}
     \item \textbf{AND} \texttt{predicted\_sex == genetic\_sex} (NOT a strict mismatch)
     ...
     \item \textbf{Sex outliers flagged}: 8 samples (0.32\% of 2,505 PCA-cleaned samples)
   \end{itemize}

\textbf{Total sex-related flags}: 31 samples (1.24\% of 2,505 PCA-cleaned samples)
...
\item \textbf{Unique samples flagged}: 84 samples (3.32\% of 2,527 analysis-ready samples)
\item \textbf{Samples flagged by multiple methods}: 16 samples (19.0\% of all flagged samples)
  \begin{itemize}
    \item 2 methods: 9 samples (10.7\% of all flagged samples)
    \item 3 methods: 7 samples (8.3\% of all flagged samples)
  \end{itemize}
\end{itemize}

\textbf{Breakdown by QC method} (all percentages relative to 2,527 analysis-ready samples):
\begin{itemize}
\item Initial QC: 0 samples (0.00\% of 2,527) - Note: Initial QC failures not tracked in refactored pipeline (pre-filtered input)
\item PCA: 22 samples (0.87\% of 2,527)
\item Sex Mismatch (Strict): 23 samples (0.91\% of 2,527)
\item Sex Outlier (Threshold): 8 samples (0.32\% of 2,527)
\item Technical: 33 samples (1.31\% of 2,527)
...
\begin{itemize}
\item \textbf{Samples passing all QC}: \textbf{2,443 samples} (96.68\% retention rate from 2,527 analysis-ready samples)
\item \textbf{Biological proteins}: 5,416 (24 control probes excluded from released data)
\item \textbf{Clean NPX matrix}: 2,443 samples $\times$ 5,416 proteins
\end{itemize}
...
\textbf{Total samples removed from raw (2,600 $\to$ 2,443)}: 157 samples (6.04\%)
\begin{itemize}
...
\item Unique outliers flagged: 84 (3.23\% of 2,600 raw, 3.32\% of 2,527 analysis-ready)
  \begin{itemize}
    \item Initial QC: 0 samples (not tracked in refactored pipeline)
    \item PCA: 22 samples
    \item Sex: 31 samples (23 strict mismatches + 8 threshold outliers)
    \item Technical: 33 samples
    \item Z-score: 7 samples
    \item pQTL: 14 samples
    \item Overlaps: 16 samples flagged by multiple methods (9 by 2 methods + 7 by 3 methods)
  \end{itemize}
\end{itemize}

\textbf{Retention rate}: 96.68\% (2,443/2,527 analysis-ready samples) or 94.69\% (2,443/2,580 biological samples)
...
\item \textbf{Structure}:
  ...
  \end{itemize}
\item \textbf{Dimensions}: 2,443 samples (rows) $\times$ 5,416 proteins (columns)
...
\item \textbf{Multiple method flags}: 16 samples (19.0\% of all outliers) were flagged by multiple methods, providing high-confidence outlier identification.
...
\item \textbf{Document version v.2.1 update (January 15, 2026)}: This version reflects the refactored pipeline implementation. Key differences from v.02:
  \begin{itemize}
    ...
    \item Technical outliers: 33 (vs 27 in v.02) - 6 additional outliers (parallel flagging detecting additional valid outliers)
    \item Sex mismatch outliers: 23 (vs 18 in v.02) - 5 additional outliers
    \item Sex threshold outliers: 8 (vs 13 in v.02) - 5 fewer outliers
    \item Total outliers: 84 (vs 86 in v.02) - 2 fewer outliers overall
    \item Final QC passed samples: 2,443 (vs 2,441 in v.02) - 2 additional samples
    \item Multiple method overlaps: 16 (vs 15 in v.02) - 1 additional overlap
  \end{itemize}
  These differences are primarily due to:
  \begin{itemize}
    \item Different input sample counts (2,527 vs 2,522) affecting threshold calculations
    \item Parallel flagging implementation enabling detection of additional valid outliers
...
\noindent
\textbf{Pipeline Version}: January 2026 (Refactored) \\
\textbf{Data Release}: FG3 Batch 2 \\
\textbf{Final Sample Count}: 2,443 samples $\times$ 5,416 proteins (24 control probes excluded from released data)\\
